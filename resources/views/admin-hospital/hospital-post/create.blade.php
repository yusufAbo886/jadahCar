...
@section('content')
    ...
    <div class="card card-custom gutter-b example example-compact">
        ...
        <form method="post" action="{{route('hospital-post.store')}}" enctype="multipart/form-data">
            @csrf

            <div class="card-body">
                <div class="row">
                    <div class="col-6 form-group category" >
                        <label for="cars">Choose a departmen:</label>

                        <select  class="form-control form-control-solid " name="category_id" id="categoryId"  required>
                            <?php foreach ($category as $key => $value){ ?>
                                <option  value="">select</option>
                            <option  value="{{$value->id}}">{{$value->theNameEn}}</option>
