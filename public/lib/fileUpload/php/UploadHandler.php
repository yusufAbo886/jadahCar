...
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->response = array();
        $this->options = array(
            'script_url' => $this->get_full_url() . '/files/' . $pageType . '/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/files/' . $pageType . '/',
            'upload_url' => $this->get_full_url() . '/files/' . $pageType . '/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|svg|mp4)$/i',
            ...
            'image_file_types' => '/\.(gif|jpe?g|png|svg|mp4)$/i',
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error, $index, $content_range) {
        ...
        if (strpos($name, '.') === false &&
                preg_match('/^video\/(gif|jpe?g|png|svg|mp4)/', $type, $matches)) {
            $name .= '.' . $matches[1];
        }
    ...
    protected function gd_create_scaled_image($file_name, $version, $options) {
        ...
        switch ($type) {
            ...
            case 'png':
                ...
                break;
            case 'svg':
                $src_func = 'imagecreatefromsvg';
                $write_func = 'imagesvg';
                $image_quality = isset($options['svg_quality']) ?
                        $options['svg_quality'] : 9;
                break;
            case 'mp4':
                $src_func = 'imagecreatefromsvg';
                $write_func = 'imagesvg';
